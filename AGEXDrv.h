...
#include <linux/of_device.h>		// f√ºr of*
#include <linux/pci.h>		// f√ºr pci*
#include <linux/spi/spi.h>		// f√ºr spi*
...
#define DMA_READ_TC_SG_MAX_BYTECOUNT ( (1<<22)-1 ) /* kˆnnen max 20Bit DWord (1MByte) */
...
typedef struct _DMA_READ_JOB
{
	...
	u32				anzSGItemsLeft;	//wie viele SGs m¸ssen noch ¸bertragen werden, bzw. dem FPGA ¸ber geben
	struct scatterlist *pSGNext;	//N‰chste Element was gesendet werden kann, kann aber auch sg_is_last()==true sein oder NULL f¸r last+1, es m¸ssen aber nicht alle Element einer SGListe benutzt sein.
...
//	unsigned long	BAR0SizeBytes;
	...
	spinlock_t				DMARead_SpinLock;	//Lock f¸r DMAStructs/und DMAUnit im FPGA
